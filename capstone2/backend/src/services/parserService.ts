import { Medicine, PrescriptionData, Reminder, MedicineSchedule } from '../types';
 ...
 */
export const parsePrescription = (rawText: string): PrescriptionData => {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      const correctedName = correctSpelling(rawName);
 ...
 */
const parseSchedule = (timings: string[], frequency: string): MedicineSchedule[] => {
  const schedules: MedicineSchedule[] = [];
  const defaultTimes: { [key: string]: { time: string; period: string } } = {
 ...
 */
const parseSimpleFormat = (lines: string[]): Medicine[] => {
  const medicines: Medicine[] = [];
  let id = 1;
  ...
  for (const line of lines) {
    ...
    if (parts.length >= 2) {
      const name = correctSpelling(parts[0]);
      medicines.push({
        id: id++,
        name,
        dosage: parts[1] || '',
 ...
 */
export const generateReminders = (medicines: Medicine[]): Reminder[] => {
